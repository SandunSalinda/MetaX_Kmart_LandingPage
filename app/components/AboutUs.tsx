...
const FeatureCard: React.FC<FeatureCardProps> = ({ iconSrc, title, description }) => (
  ...
  <div className="w-full md:w-[380px] h-48 sm:h-52 md:h-60 p-4 sm:p-5 md:p-6 bg-white rounded-xl shadow-[0_4px_18px_0_rgba(0,0,0,0.13)] flex flex-col justify-start items-start gap-4 sm:gap-5 md:gap-6">
  ...
  return (
    ...
    <section id="about-us" className="mx-auto max-w-[1199px] px-4 pt-0 pb-16 flex flex-col items-center justify-center"> {/* Changed pb-28 to pb-16 */}
      ...
      <div className="w-full max-w-[896px] flex flex-col items-center justify-center gap-4 sm:gap-5 md:gap-6 text-center mb-8 sm:mb-10 md:mb-12"> {/* Responsive gaps and margins */}
        ...
        <h2 className="text-neutral-800 text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl xl:text-[64px] font-semibold font-['Plus_Jakarta_Sans'] leading-tight md:leading-[104px] px-4">
          Why Shop with K Mart?
        </h2>
        {/* Responsive subheading sizing */}
        <p className="text-neutral-500 text-sm sm:text-base md:text-lg lg:text-xl font-normal font-['Plus_Jakarta_Sans'] px-4">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 justify-items-center px-2 sm:px-0">
        <FeatureCard
